...
def isWinner(bo, le):
    ...
    the player has won.
    We us "bo" instead of "board" and "le" instead of "letter" so we
...
def getComputerMove(board, computerLetter):
    '''Given the board and the computer's letter, determine where to move
    ...
    # Here is the algorithm for out Tic-Tac-Toe AI:
...
def isBoardFull(board):
    '''Returns True if every space on the board has been taken.  Otherwise
    return False.'''
    for i in range(1, 10):
        if isSpaceFree(board, i):
            return False
